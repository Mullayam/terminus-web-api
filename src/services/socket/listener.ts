...
export class SocketListener {
    ...
    constructor(
        ...
        sftp.on("upload", (info) =>
            this.io.emit(SocketEventConstants.FILE_UPLOADED, info.destination),
        );
    ...
    public onConnection(socket: Socket) {
        ...
        if (sessionId) {
            const existingInfo = this.sessionInfo[sessionId];

            if (existingInfo) {
                // Reconnect: same session, new socket — swap admin socket ID
                const oldSocketId = existingInfo.adminSocketId;
                if (oldSocketId && oldSocketId !== socket.id) {
                    existingInfo.connectedSockets?.delete(oldSocketId);
                    existingInfo.socketPermissions?.delete(oldSocketId);
                    // Kick the stale socket if still connected
                    const staleSocket = this.io.sockets.sockets.get(oldSocketId);
                    if (staleSocket) {
                        staleSocket.emit(
                            SocketEventConstants.SESSIONN_END,
                            "Session taken over by a new connection",
                        );
                        staleSocket.disconnect(true);
                    }
                    Logging.dev(
                        `♻️ Session ${sessionId} reconnected: ${oldSocketId} → ${socket.id}`,
                    );
                }
                existingInfo.adminSocketId = socket.id;
                existingInfo.connectedSockets?.add(socket.id);
                // Re-join the terminal room so broadcasts still reach this socket
        ...
        Sftp_Service.emitSftpEvent(socket);
        ...
        socket.on("disconnecting", (reason) => {
            ...
            if (info.adminSocketId) {
                this.io
                    .to(info.adminSocketId)
                    .emit(SocketEventConstants.SSH_DISCONNECTED, socket.id);
            }
        ...
        socket.on("disconnect", () => {
            ...
            for (const [sessionId, info] of Object.entries(this.sessionInfo)) {
                ...
                if (socket.id === info.adminSocketId) {
                    ...
                    // End the global SFTP connection since the SSH channel is gone
    ...
    private terminalSharingSession(socket: Socket) {
        socket.on(SocketEventConstants.CreateTerminal, (session_id: string) => {
            if (!this.sessions.has(session_id)) {
                return this.io
                    .to(socket.id)
                    .emit(SocketEventConstants.session_not_found, "Session not found");
            }
            ...
            const existingSocketIds =
                this.sharedTerminalSessions.get(session_id) || [];
            ...
            if (!existingSocketIds.includes(socket.id)) {
                ...
                info?.adminSocketId &&
                    this.io
                        .to(info?.adminSocketId)
                        .emit(SocketEventConstants.session_info, {
                            socketId: socket.id,
    ...
    private async sshOperation(socket: Socket) {
        ...
        const resume = async () => {
            ...
            ssh.on("error", (err) => {
                socket.emit(
                    SocketEventConstants.SSH_EMIT_ERROR,
            ...
            if (info) {
                ...
                if (targetSocket) {
                    switch (data.type) {
                        case "pause":
                            targetSocket.emit(
                                SocketEventConstants.session_info,
                                `Your session has been ${data.type} by an admin, click "Resume" to continue`,
                            );
                        ...
                        case "kick":
                            targetSocket.emit(
                                SocketEventConstants.SESSIONN_END,
                                `Your session has been terminated by an admin`,
                            );
        ...
        socket.on(SocketEventConstants.SSH_PERMISSIONS, async (input: string) => {
            ...
            if (info) {
                ...
                this.io.sockets.sockets
                    .get(data.socketId)
                    ?.emit(SocketEventConstants.SSH_PERMISSIONS, input);
                info.adminSocketId &&
                    this.io.sockets.sockets
                        .get(info.adminSocketId)
                        ?.emit(SocketEventConstants.SSH_PERMISSIONS, input);
            }
        });
        ...
        socket.on(SocketEventConstants.SSH_START_SESSION, async (input: string) => {
            ...
            let conn: Client =
                this.sessions.get(sessionId) || new Client({ captureRejections: true });
            ...
            conn.on("ready", function () {
                ...
                conn.shell(
                    {
                        cols: _this.sessionInfo[sessionId]?.terminalSize?.cols || 150,
                        rows: _this.sessionInfo[sessionId]?.terminalSize?.rows || 40,
                        term: "xterm-256color",
                    },
                    function (err, stream) {
                        if (err) {
            ...
            conn.on("banner", (data) => {
                socket.emit(
                    SocketEventConstants.SSH_BANNER,
                    data.replace(/\r?\n/g, "\r\n").toString(),
                );
            });
    ...
    sftpOperation(socket: Socket) {
        ...
        socket.on(
            SocketEventConstants.SFTP_ZIP_EXTRACT,
            async (payload: FileOperationPayload): Promise<any> => {
                try {
        ...
        socket.on(
            SocketEventConstants.SFTP_GET_FILE,
            async (payload: FileOperationPayload): Promise<any> => {
                try {
                    let dirPath: string | undefined = payload?.dirPath;
        ...
        // File Properties
        socket.on(
            SocketEventConstants.SFTP_EXISTS,
            async (payload: FileOperationPayload): Promise<any> => {
                const { dirPath } = payload;
                if (!dirPath)
                    return socket.emit(
                        SocketEventConstants.ERROR,
        ...
        socket.on(
            ...
            async (payload: FileOperationPayload): Promise<any> => {
                ...
                if (!oldPath || !newPath)
                    return socket.emit(SocketEventConstants.ERROR, "Invalid file paths");
        ...
        socket.on(
            ...
            async (payload: FileOperationPayload): Promise<any> => {
                ...
                if (!filePath)
                    return socket.emit(SocketEventConstants.ERROR, "Invalid file path");
        ...
        socket.on(
            ...
            async (payload: FileOperationPayload): Promise<any> => {
                ...
                if (!folderPath)
                    return socket.emit(SocketEventConstants.ERROR, "Invalid folder path");
        ...
        socket.on(
            SocketEventConstants.SFTP_FILE_DOWNLOAD,
            async (payload: FileOperationPayload): Promise<any> => {
                const { path } = payload;
                if (!path)
                    return socket.emit(SocketEventConstants.ERROR, "Invalid  path");
        ...
        socket.on(
            SocketEventConstants.SFTP_FILE_STATS,
            async (payload: FileOperationPayload): Promise<any> => {
                const { path } = payload;
                if (!path)
                    return socket.emit(SocketEventConstants.ERROR, "Invalid  path");
        ...
        socket.on(
            SocketEventConstants.SFTP_EDIT_FILE_REQUEST,
            async (payload: { path: string }): Promise<any> => {
                ...
                try {
                    ...
                    socket.emit(
                        SocketEventConstants.SFTP_EDIT_FILE_REQUEST_RESPONSE,
                        content,
                    );
                } catch (error: any) {
                    socket.emit(SocketEventConstants.ERROR, error.message || "Error reading file");
                }
            },
        );
        // Edit file content
        socket.on(
            SocketEventConstants.SFTP_EDIT_FILE_DONE,
            async (payload: EditFilePayload): Promise<any> => {
    ...
    private connectSFTP(socket: Socket) {
        socket.on(SocketEventConstants.SFTP_CONNECT, async (data: any) => {
            try {
                // Always reconnect if the connection is dead
              
                if (!Sftp_Service.is_connected) {
                    await Sftp_Service.connectSFTP(this.sshConfig(data) as any,  socket.handshake.query.sessionId as string);
                }

                if (!Sftp_Service.is_connected) {
                    socket.emit(SocketEventConstants.SFTP_EMIT_ERROR, 'Failed to connect to SFTP server');
                    return;
                }
                ...
                socket.emit(SocketEventConstants.SFTP_EMIT_ERROR, 'SFTP connection error: ' + err.message);
            }
        });
    }
